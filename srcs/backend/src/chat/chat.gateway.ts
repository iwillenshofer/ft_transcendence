...
export class ChatGateway implements OnGatewayConnection, OnGatewayDisconnect {
  ...
  async handleConnection(socket: Socket, ...args: any[]) {
    const user = await this.userService.getUserById(+socket.handshake.headers.userid);
    ...
    let connectedUser = await this.connectedUserService.getByUserId(user.id);
    if (connectedUser == null)
      await this.connectedUserService.createConnectedUser(socket.id, user);
    else {
      await this.connectedUserService.updateSocketIdConnectedUSer(socket.id, connectedUser);
    }
  ...
  async createRoom(socket: Socket, createRoomDto: CreateRoomDto) {
    ...
    const owner = await this.userService.getUser(+socket.handshake.headers.userid);
    ...
    const connectedUsers = await this.connectedUserService.getAllConnectedUser();
  ...
  //   const user: UserDTO = await this.userService.getUser(joinRoomDto.userId);
